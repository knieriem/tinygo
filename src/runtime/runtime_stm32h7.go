...
func initCLK() {
	...
	selectVoltageScaling(voltageScale2)
	...
	setFlashLatency(1, ifSlowingDown)
	setAPB3Div(APB3Div, ifSlowingDown)
	setAPB1Div(APB1Div, ifSlowingDown)
	setAPB2Div(APB2Div, ifSlowingDown)
	setAPB4Div(APB4Div, ifSlowingDown)
	setHPREDiv(HPREDiv, ifSlowingDown)

	setSysclkDiv(SysclkDiv)

	setSysclkSrc(stm32.RCC_CFGR_SW_HSI)

	setHPREDiv(HPREDiv, ifSpeedingUp)
	setAPB3Div(APB3Div, ifSpeedingUp)
	setAPB1Div(APB1Div, ifSpeedingUp)
	setAPB2Div(APB2Div, ifSpeedingUp)
	setAPB4Div(APB4Div, ifSpeedingUp)
	setFlashLatency(1, ifSpeedingUp)
}

func selectVoltageScaling(scaleSpec uint) {

	stm32.PWR.D3CR.ReplaceBits(uint32(scaleSpec)<<stm32.PWR_D3CR_VOS_Pos, stm32.PWR_D3CR_VOS_Msk, 0)
...
func setSysclkSrc(sel uint32) {
	...
	for stm32.RCC.CFGR.Get()&stm32.RCC_CFGR_SWS_Msk != sel {
	}
}
...
func ifSlowingDown(want, have uint32) bool {
	return want > have
}

func ifSpeedingUp(want, have uint32) bool {
	return want < have
}
