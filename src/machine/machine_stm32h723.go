...
// see RM0468, 66.1 Unique device ID register, p. 3315
const uidBase = 0x1FF1_E800

var deviceIDAddr = []uintptr{uidBase, uidBase + 4, uidBase + 8}

// Internal use: configured speed of the APB1 and APB2 timers, this should be kept
// in sync with any changes to runtime package which configures the oscillators
// and clock frequencies
const APB1_TIM_FREQ = 96e6
const APB2_TIM_FREQ = 96e6
...
func (p Pin) registerInterrupt() interrupt.Interrupt {
	...
	case 4:
		return interrupt.New(stm32.IRQ_EXTI9_5, func(interrupt.Interrupt) { handlePinInterrupt(4) })
	case 5:
...
func (uart *UART) getBaudRateDivisor(baudRate uint32) uint32 {
	...
	switch uart.Bus {
	case stm32.USART1:
		clock = CPUFrequency() / 2 / 2 // APB2 Frequency
		rate = uint32(clock / baudRate)
	case stm32.USART2:
		clock = CPUFrequency() / 2 / 2 // APB1 Frequency
		rate = uint32(clock / baudRate)
	case stm32.USART3:
		clock = CPUFrequency() / 2 / 2 // APB1 Frequency
		rate = uint32(clock / baudRate)
