...
func writeGo(outdir string, device *Device, interruptSystem string, periphAliases map[string]string) error {
	...
	t := template.Must(template.New("go").Funcs(template.FuncMap{
		...
		"splitLine":   splitLine,
		"alias": func(prevName string) string {
			return periphAliases[prevName]
		},
...
func generate(indir, outdir, sourceURL, interruptSystem, periphAliasDefs string) error {
	periphAliases, err := setupPeriphAliasMap(periphAliasDefs)
	if err != nil {
		return err
	}
	...
	for _, infile := range infiles {
		...
		err = writeGo(outdir, device, interruptSystem, periphAliases)
		if err != nil {
...
func setupPeriphAliasMap(defs string) (map[string]string, error) {
	defs = strings.TrimSpace(defs)
	if defs == "" {
		return nil, nil
	}
	defList := strings.Split(defs, ",")
	m := make(map[string]string, len(defList))
	for _, def := range defList {
		f := strings.Split(def, "=")
		if len(f) != 2 {
			return nil, fmt.Errorf("malformed periph-alias definition")
		}
		alias := strings.TrimSpace(f[0])
		prev := strings.TrimSpace(f[1])
		m[prev] = alias
	}
	return m, nil
}
...
func main() {
	...
	periphAliases := flag.String("periph-alias", "", "comma separated list of aliasName=prevName expressions")
	flag.Parse()
	if flag.NArg() != 2 {
	...
	indir := flag.Arg(0)
	outdir := flag.Arg(1)
	err := generate(indir, outdir, *sourceURL, *interruptSystem, *periphAliases)
	if err != nil {
